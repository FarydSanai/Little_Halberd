...
namespace LittleHalberd
{
    ...
    public class Attack : CharacterAbility
    {
        ...
        {
            PoolObjectLoader.Instance.GetObject(ObjectType.SFX_ATTACK);
            if (characterState.AI_DATA.isRage)
            {
                characterState.ATTACK_DATA.AttackDamage *= 1.3f;
            }
        }
            ...
            if (projectiles.Length > 0)
            {
                    ...
                    stateInfo.normalizedTime <= 0.9f)
                {
                    PoolObjectLoader.Instance.GetObject(ObjectType.VFX_BOMB_REPEL,
                                                        ...
                                                        Quaternion.identity);
                    PoolObjectLoader.Instance.GetObject(ObjectType.SFX_REPEL_PROJECTILE);
